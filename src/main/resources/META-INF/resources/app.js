...
function refreshTimeTable() {
    $.getJSON("/timeTable", function (timeTable) {
        ...
        timeTable.lessonList.forEach(lesson => {
            if(lesson.stream.length > 1){
                lesson.stream.forEach(subclass => {
                    timeTable.lessonList.push({
                        "id": parseInt(646+''+subclass.id),
                        "subjectId": lesson.subjectId,
                        "teacherId": lesson.teacherId,
                        "streamId": `${subclass.id}`,
                        "className": [subclass.className],
                        "stream": [subclass],
                        "classSubjects": lesson.classSubjects,
                        "teachers": lesson.teachers,
                        "timeslot": lesson.timeslot,
                        "dayAllocated": lesson.dayAllocated,
                        "teacherNames": lesson.teacherNames,
                        "subjects": lesson.subjects,
                        "groups": `${subclass.className}`
                    })
                })
                removeLesson(lesson)
            }
        ...
        function removeLesson(lesson){
            timeTable.lessonList = timeTable.lessonList.filter(lessonData => lessonData.id !== lesson.id)
        }
        ...
        const teacherList = [...new Set(timeTable.lessonList.map(lesson => lesson.teacherNames))];
        ...
        const studentGroupList = [...new Set(timeTable.lessonList.map(lesson => lesson.groups))];
        ...
        $.each(timeTable.lessonList, (index, lesson) => {
            ...
            if (lesson.timeslot == null) {
                ...
                $(`#timeslot${lesson.timeslot.id}teacher${convertToId(lesson.teacherNames)}`).append(lessonElementWithoutDelete.clone());
                $(`#timeslot${lesson.timeslot.id}studentGroup${convertToId(lesson.groups)}`).append(lessonElementWithoutDelete.clone());
            }
        });
    });
}
